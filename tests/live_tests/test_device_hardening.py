...
def test_validate_device_hardening_expects_secure_management_posture(
    ...
    assert_that(
        ...
        description="live validation should confirm SSH is enabled",
    ).is_true()
    assert_that(
        live_validation_result["validation"]["telnet_present"],
        description="live validation should confirm Telnet is disabled",
    ).is_false()
    assert_that(
        live_validation_result["validation"]["banner_present"],
        description="live validation should confirm a login banner is configured",
    ).is_true()
    assert_that(
        live_validation_result["validation"]["passed"],
        description="live hardening validation should pass for the default mocked running config",
    ).is_true()
